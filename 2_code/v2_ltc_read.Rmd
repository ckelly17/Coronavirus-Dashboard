...
######################################################
## set google access
gs4_auth(email = "srmclaughlin41@gmail.com") # may need to run this directly from the console, not sure why
...
ltc2 <- ltc2 %>% 
  ...
  mutate(totalDeathsAllLTC = DeathRes_ltc	+ ProbDeathRes_ltc + DeathStaff_ltc	+ ProbDeathStaff_ltc + DeathResStaff_ltc	+ ProbDeathResStaff_ltc +
         ...
         # positives by Resident
         totalPosAllLTC_Res = PosRes_ltc	+ ProbPosRes_ltc + 
           PosRes_nh	+ ProbPosRes_nh + 
           PosRes_alf	+ ProbPosRes_alf + 
           PosRes_other	+ ProbPosRes_other,
         
         # positives by Staff
         totalPosAllLTC_Staff = PosStaff_ltc	+ ProbPosStaff_ltc +
           PosStaff_nh	+ ProbPosStaff_nh + 
           PosStaff_alf	+ ProbPosStaff_alf + 
           PosStaff_other	+ ProbPosStaff_other,
         ...
         # positives in different facility types by Resident
         totalPosNH_Res = PosRes_nh	+ ProbPosRes_nh, 
         totalPosALF_Res = PosRes_alf	+ ProbPosRes_alf,
         totalPosLTC_Res = PosRes_ltc	+ ProbPosRes_ltc,
         totalPosOther_Res = PosRes_other	+ ProbPosRes_other,
         
         # positives in different facility types by Staff 
         totalPosNH_Staff = PosStaff_nh	+ ProbPosStaff_nh,
         totalPosALF_Staff = PosStaff_alf	+ ProbPosStaff_alf,
         totalPosLTC_Staff = PosStaff_ltc	+ ProbPosStaff_ltc,
         totalPosOther_Staff = PosStaff_other	+ ProbPosStaff_other,
  ...
  mutate(obs = row_number(), # counts the observation number within each group, can't lag the first observation
         ...
         # change in positives, by Res                        
         newPosAllLTC_Res = if_else(obs == 1, totalPosAllLTC_Res, totalPosAllLTC_Res - lag(totalPosAllLTC_Res)),
         newPosNH_Res = if_else(obs == 1, totalPosNH_Res, totalPosNH_Res - lag(totalPosNH_Res)),
         newPosALF_Res = if_else(obs == 1, totalPosALF_Res, totalPosALF_Res - lag(totalPosALF_Res)),
         newPosLTC_Res = if_else(obs == 1, totalPosLTC_Res, totalPosLTC_Res - lag(totalPosLTC_Res)),
         newPosOther_Res = if_else(obs == 1, totalPosOther_Res, totalPosOther_Res - lag(totalPosOther_Res)),
         
         # change in positives, by Staff                       
         newPosAllLTC_Staff = if_else(obs == 1, totalPosAllLTC_Staff, totalPosAllLTC_Staff - lag(totalPosAllLTC_Staff)),
         newPosNH_Staff = if_else(obs == 1, totalPosNH_Staff, totalPosNH_Staff - lag(totalPosNH_Staff)),
         newPosALF_Staff = if_else(obs == 1, totalPosALF_Staff, totalPosALF_Staff - lag(totalPosALF_Staff)),
         newPosLTC_Staff = if_else(obs == 1, totalPosLTC_Staff, totalPosLTC_Staff - lag(totalPosLTC_Staff)),
         newPosOther_Staff = if_else(obs == 1, totalPosOther_Staff, totalPosOther_Staff - lag(totalPosOther_Staff)),
...
##ZZZtest sheet
sheet_write(ltc2, ss = "https://docs.google.com/spreadsheets/d/1gn_jdHlgFU0gUuOiPBkEgIyn9YszHOjUbtqAwPRbOQE/edit#gid=0", sheet = "LTC2")
