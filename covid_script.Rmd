...
# MERGE FUNCTION
######################################################
merge_tabler <- function(df_1, df_2, keys){
  # FIRST, INNER_JOIN RETURNS ALL KEY COMBINATIONS THAT APPEAR IN BOTH DATASETS
  m1 <- inner_join(df_1, df_2, by = keys) %>%
    mutate(merge = "both") # ROWS THAT APPEAR IN BOTH DATASETS
  
  # NEXT, ANTI_JOIN SHOWS KEY COMBOS PRESENT IN DF_1 THAT ARE NOT PRESENT IN DF_2
  m2 <- bind_rows(m1, anti_join(df_1, df_2, by = keys) %>% 
                    mutate(merge = 'left_only')) # ROWS THAT ONLY APPEAR IN DF_1 
  
  # THEN, ANTI_JOIN SHOWS KEY COMBOS PRESENT IN DF_2 THAT ARE NOT PRESENT IN DF_1
  m3 <- bind_rows(m2, anti_join(df_2, df_1, by = keys) %>% 
                    mutate(merge = 'right_only')) # ROWS THAT ONLY APPEAR IN DF_2 
...
varnames <- c("fips", "stname", "ctyname","pop", "cases", "deaths", "caseper1k", "deathper1k", "date", "msaname", "cbsaname")
...
cty_election <- cty_election %>% 
  ...
  dplyr::rename(fips = combined_fips) %>% 
...
# read data
tracking <- fromJSON("https://covidtracking.com/api/v1/states/daily.json")

# select columns
tracking <- tracking %>%
  select(date, state, fips, positive, negative, hospitalizedIncrease, hospitalizedCurrently, totalTestResultsIncrease, positiveIncrease, negativeIncrease, deathIncrease, totalTestResults, death, inIcuCumulative, inIcuCurrently, onVentilatorCurrently, onVentilatorCumulative, positiveCasesViral)
...
tracking <- tracking %>%
  ...
  mutate(positiveCasesViralIncrease = positiveCasesViral - lag(positiveCasesViral),
         ...
         death_7d_avg = ifelse(state == "NJ" & date >= "2020-06-25" & date <= "2020-07-01", (death_7d_avg - 264.85), death_7d_avg),
         death_7d_avg = ifelse(state == "TX" & date >= "2020-07-27" & date <= "2020-08-02", (death_7d_avg - 90.14), death_7d_avg),
...
covid_data_states <- covid_data_states %>%
  ...
  mutate(pop = max(pop, na.rm = TRUE),
        caseper1k = positive/pop*1000, 
        deathper1k = deaths/pop*1000,
        max_caseper1k = max(caseper1k),
        state_tier = ifelse(max_caseper1k > 15, "Tier 1", ifelse((max_caseper1k > 10), "Tier 2", ifelse(max_caseper1k > 5, "Tier 3","Tier 4"))),
        state_tier_moving = ifelse(caseper1k > 15, "15 or more cases per 1k", ifelse((caseper1k > 10), "10-15 cases per 1k", ifelse(caseper1k > 5, "5-9 cases per 1k","Less than 5 cases per 1k"))))
...
mobility <- mobility %>%
  ...
  dplyr::rename(state = sub_region_1, county = sub_region_2) %>%
  mutate(county = ifelse(is.na(county), "All Counties", county),
         state = ifelse(is.na(state), "All States", state))

states <- covid_data_states %>% select(state, state_abb)
