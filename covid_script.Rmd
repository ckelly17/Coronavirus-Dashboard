...
msa <- msa %>%
  dplyr::select(fipscounty, cbsaname, msaname) %>%
...
# select columns
tracking <- tracking %>%
  dplyr::select(date, state, fips, positive, negative, hospitalizedIncrease, hospitalizedCurrently, totalTestResultsIncrease, positiveIncrease, negativeIncrease, deathIncrease, totalTestResults, death)
...
covid_data_states <- covid_data_states %>%
  ...
  mutate(pop = max(pop, na.rm = TRUE),
        ...
        state_tier = ifelse(max_caseper1k > 15, "Tier 1", ifelse((max_caseper1k > 9), "Tier 2", ifelse(max_caseper1k > 3, "Tier 3","Tier 4"))),
        state_tier_moving = ifelse(caseper1k > 15, "15 or more cases per 1k", ifelse((caseper1k > 9), "9-15 cases per 1k", ifelse(caseper1k > 3, "3-9 cases per 1k","Less than 3 cases per 1k"))))
...
covid_data_states <- covid_data_states %>%
  mutate(HospAprFlag = ifelse(hosp_7d_avg > 0 & date == "2020-04-07", 1, 0),
          HospAprFlag = ifelse(is.na(HospAprFlag), 0, HospAprFlag),
         HospMayFlag = ifelse(hosp_7d_avg > 0 & date == "2020-05-07", 1, 0),
          HospMayFlag = ifelse(is.na(HospMayFlag), 0, HospMayFlag),
         HospEndFlag = ifelse(hosp_7d_avg > 0 & date == (today() - 1), 1, 0)) %>%
  group_by(state) %>%
  mutate(HospAprMay = sum(HospAprFlag, HospEndFlag),
         HospMay = sum(HospMayFlag, HospEndFlag))
